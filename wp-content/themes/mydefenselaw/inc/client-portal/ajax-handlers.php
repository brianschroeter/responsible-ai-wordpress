...
function mydefenselaw_portal_ajax_mark_read() {
	...
	if (!$message_id) {
		wp_send_json_error(array('message' => __('Invalid message ID', 'mydefenselaw')));
	}

	// 4. Verify message exists
	$message = get_post($message_id);
	if (!$message || $message->post_type !== 'client_message') {
		wp_send_json_error(array('message' => __('Message not found', 'mydefenselaw')));
	}

	// 5. Verify user can access this message
	if (!mydefenselaw_portal_can_access_message($user_id, $message_id)) {
		wp_send_json_error(array('message' => __('Access denied', 'mydefenselaw')));
	}

	// 6. Mark as read
	update_post_meta($message_id, '_is_read', true);
	update_post_meta($message_id, '_read_at', current_time('mysql'));
	update_post_meta($message_id, '_read_by', $user_id);

	if (function_exists('update_field')) {
		update_field('is_read', true, $message_id);
		update_field('read_at', current_time('Y-m-d H:i:s'), $message_id);
	}

	// 7. Log activity
	mydefenselaw_portal_log_activity('message_read', 'message', $message_id);

	wp_send_json_success(array(
		'message' => __('Message marked as read', 'mydefenselaw'),
	));
}
